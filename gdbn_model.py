...
class iDBN:
    """
    Unimodal stacked RBM wrapper compatible with your main code.
    Provides:
      - layer-wise training via RBM.train_epoch
      - represent(x): propagate input and return top-level representation
      - reconstruct(x): full reconstruction
    """
    def __init__(self, layer_sizes, params, dataloader, val_loader, device,
        ...
        # Validation images/labels (may be images or one-hot labels)
        if val_loader is not None:
            self.val_batch, val_labels = next(iter(val_loader))
            # choose appropriate modality view for preview
            
            # extract the features used to track the formation of embeddings 
            ...
            #self.fa = [val_loader.dataset.dataset.FA_list[i] for i in indices]
            self.features = {
                ...
                "Labels": self.labels,
                #"FA": torch.tensor(self.fa, dtype=torch.float)
                }
        ...
        # Build RBM layers: layer_sizes is a list like [input_dim, h1, h2, ...]
        for i in range(len(layer_sizes) - 1):
            
            # Parametri di base, sempre passati
            rbm_params = {
                'num_visible': layer_sizes[i],
                'num_hidden': layer_sizes[i + 1],
                'learning_rate': params['LEARNING_RATE'],
            ...
            # Se è l'ultima iterazione, aggiungi i parametri di sparsity
            if i == len(layer_sizes) - 2:
                rbm_params['sparsity'] = self.sparsity
                rbm_params['sparsity_factor'] = self.sparsity_factor

            # Inizializza l'RBM con i parametri definiti
            rbm = RBM(**rbm_params)
    ...
    # -----------------
    # Layer-wise training (keeps same API as before)
    # -----------------
    def train(self, epochs, run_id=0):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for train_data, labels in self.dataloader:
                # choose modality depending on text_flag
                ...
                for rbm_layer in self.layers:
                    ...
                    # forward through rbm to get hidden probs for next layer
            ...
            # Log reconstructions for image DBN only (if validation exists)
            if epoch % 5 == 0 and (not self.text_flag) and (self.validation_images is not None):
                with torch.no_grad():
                    ...
                    # validation_images may be shaped [B, C, H, W] or flattened; adapt
                    try:
                    ...
                    try:
                        ...
                        pass
            # --- Aggiungi questo blocco per il plotting degli embeddings ---
            if self.wandb_run and self.features and epoch % 10 == 0:  # Plotta ogni 10 epoche
                print(f"Plotting embeddings at epoch {epoch}...")
                # Calcola l'embedding finale per tutto il set di validazione
                with torch.no_grad():
                    final_embedding = (self.represent(self.val_batch)).detach().cpu()
                
                # Riduci la dimensionalità con PCA e chiama la funzione esterna
                if final_embedding.shape[0] > 1 and final_embedding.shape[1] > 2:
                    ...
                    #emb_2d_umap = Umap.fit_transform(final_embedding)
    ...
    def reinitialize_layers(self):
        # rebuild layers with same sizes and device
    ...
    def represent(self, x: torch.Tensor) -> torch.Tensor:
        """
        Propaga l'input attraverso tutti gli RBM e ritorna la rappresentazione top-level.
        x can be already flattened or shaped as (B, C, H, W) or (B, D)
        """
        with torch.no_grad():
            v = x.view(x.size(0), -1).float().to(self.device)
            for rbm in self.layers:
                v = rbm.forward(v)  # hidden probabilities
        return v

    # -----------------
    def reconstruct(self, x: torch.Tensor) -> torch.Tensor:
        """
        Ricostruisce x passando avanti fino al top e poi indietro.
        Restituisce un tensor della stessa forma (flattened) dell'input usato in represent.
        """
        with torch.no_grad():
            v = x.view(x.size(0), -1).float().to(self.device)
            # forward
            ...
            # cur is reconstruction in visible space (flattened)
            return cur.view(v.size(0), -1)

    # -----------------
    def log_images(self, writer, original, reconstructed, epoch):
        # expects original and reconstructed as image tensors [B, C, H, W]
